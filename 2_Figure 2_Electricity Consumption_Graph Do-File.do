...
collapse (mean) hghinc_mean=kwh, by(syear hghinc_quintile) 
...
twoway (line hghinc_mean syear if hghinc_quintile == 1, sort lcolor(blue) lwidth(medium) legend(order(1 "Q1"))) ///
        ...
        ytitle("Electricity Consumption (in kWh)") 
        ylabel(, labsize(small) angle(0) grid glcolor(gs12) glpattern(solid))///
        xtitle("Year") ///
        title("Average Electricity Consumption (in kWh)" "by Income Quintiles", size(medium) color(black)) ///
